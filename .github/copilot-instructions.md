...
- **Core Components:**
  - `Modules/Microsoft365DSC`: Contains DSC resources for individual Microsoft 365 services (e.g., Exchange, Teams, SharePoint) and other core modules shared across the Microsoft365DSC module.
    ...
    - `Modules/EncodingHelpers/M365DSCEmojis.psm1`: Central definitions of emoji/Unicode glyphs as `$Global:M365DSCEmoji*` variables (built via `[char]::ConvertFromUtf32`). Loaded as a nested module from `Microsoft365DSC.psd1`.
...
25. **Reuse cached data; do not re-query the service.** Honour the `$Script:exportedInstance`/`$Script:exportedInstances` cache (comparing against the `[Key]` property) in `Get-TargetResource`, and follow the existing lazily-initialized, name-keyed script-scope cache pattern (e.g., `$Script:CompareParametersCache`, `$Script:AllM365DSCResources`) for expensive reusable lookups. See `m365dsc-common-pattern.instructions.md`.
...
3. **Every `.schema.mof` parameter must have a `Description` ending with a period `.`.** Descriptions drive auto-generated wiki documentation and must be present and properly punctuated for all parameters. This includes the standard authentication-block boilerplate: historically the `Credential` (e.g. `"Credentials of the Intune Admin"`) and `CertificatePassword` (`"Username can be made up to anything but password will be used for CertificatePassword"`) descriptions lacked a trailing period — these must also end with a period, and the generator template (`ResourceGenerator/Templates/Schema.Template.mof`) and `ResourceGenerator/M365DSCResourceGenerator.psm1` emit them with the period.
