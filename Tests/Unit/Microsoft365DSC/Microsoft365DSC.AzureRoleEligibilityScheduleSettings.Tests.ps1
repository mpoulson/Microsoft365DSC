    ...
    InModuleScope -ModuleName $Global:DscHelper.ModuleName -ScriptBlock {
        ...
        Context -Name 'Get-TargetResource caches roleDefinitionId and skips redundant roleDefinitions API calls' -Fixture {
            ...
            It 'Should call the roleDefinitions API once per distinct role name' {
                ...
                # The global roleDefinitions mock (no ParameterFilter on role name) handles any role name query,
                # so 'Contributor' receives a valid response and its ID is cached under a different key.
                $testParams2 = @{
