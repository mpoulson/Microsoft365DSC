...
function Get-TargetResource
{
    ...
    try
    {
        ...
        {
            $deviceCim = New-CimInstance -ClassName 'MSFT_IntuneDeviceIdentifier' -Property @{
                Id                          = $device.id
                ...
                Platform                    = if ($device.platform) { $device.platform.ToLower() } else { '' }
            } -ClientOnly
            $deviceArray += $deviceCim
        }
...
function Test-TargetResource
{
    ...
    Write-Verbose -Message "Testing configuration of IntuneCorporateDeviceIdentifier"
    ...
    # If Ensure states differ, return false
    if ($CurrentValues.Ensure -ne $Ensure)
    {
        Write-Verbose -Message "Test-TargetResource returned $false - Ensure mismatch"
        return $false
    }
    ...
    if ($Ensure -eq 'Absent')
    {
        if ($null -ne $CurrentValues.Devices -and $CurrentValues.Devices.Count -gt 0)
        {
            Write-Verbose -Message "Test-TargetResource returned $false - Devices exist but Ensure is Absent"
            return $false
        }
        Write-Verbose -Message "Test-TargetResource returned $true - No devices and Ensure is Absent"
        return $true
    }

    # If no devices specified in desired state, consider in desired state
    if ($null -eq $Devices -or $Devices.Count -eq 0)
    {
        if ($null -eq $CurrentValues.Devices -or $CurrentValues.Devices.Count -eq 0)
        {
            Write-Verbose -Message "Test-TargetResource returned $true - No devices in current or desired state"
            return $true
        }
        else
        {
            Write-Verbose -Message "Test-TargetResource returned $false - Current has devices but desired has none"
            return $false
        }
    }

    # Check if counts match
    if ($Devices.Count -ne $CurrentValues.Devices.Count)
    {
        Write-Verbose -Message "Test-TargetResource returned $false - Device count mismatch (Current: $($CurrentValues.Devices.Count), Desired: $($Devices.Count))"
        return $false
    }
    ...
    {
        $desiredDevices += @{
            importedDeviceIdentifier   = if ($device.importedDeviceIdentifier) { $device.importedDeviceIdentifier.Trim().ToLower() } else { '' }
            importedDeviceIdentityType = if ($device.importedDeviceIdentityType) { $device.importedDeviceIdentityType } else { '' }
    ...
    # Check if all desired devices exist in current
    foreach ($desiredDevice in $desiredDevices)
    {
        $found = $false
        foreach ($currentDevice in $currentDevices)
        ...
        if (-not $found)
        {
            Write-Verbose -Message "Test-TargetResource returned $false - Device not found: $($desiredDevice.importedDeviceIdentifier)"
            return $false
        }
    }

    Write-Verbose -Message "Test-TargetResource returned $true - All devices match"
    return $true
}
