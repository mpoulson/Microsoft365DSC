...
function Get-TargetResource
{
    ...
    try
    {
        ...
        $condition = Get-MgPolicyPermissionGrantPolicyExclude `
...
function Set-TargetResource
{
    ...
    $currentValues = Get-TargetResource @PSBoundParameters
        ...
        try
        {
            ...
            {
                $null = New-MgPolicyPermissionGrantPolicyExclude -PermissionGrantPolicyId $PermissionGrantPolicyId `
                    -BodyParameter $body -ErrorAction Stop
            }
            elseif ($Ensure -eq 'Present' -and $currentValues.Ensure -eq 'Present')
            {
                $null = Remove-MgPolicyPermissionGrantPolicyExclude -PermissionGrantPolicyId $PermissionGrantPolicyId `
                    -PermissionGrantConditionSetId $Id -ErrorAction Stop
                $null = New-MgPolicyPermissionGrantPolicyExclude -PermissionGrantPolicyId $PermissionGrantPolicyId `
                    -BodyParameter $body -ErrorAction Stop
            }
...
function Export-TargetResource
{
    ...
    try
    {
        ...
        {
            $excludes = Get-MgPolicyPermissionGrantPolicyExclude -PermissionGrantPolicyId $policy.Id -All -ErrorAction Stop
            $index = 1
            foreach ($exclude in $excludes)
            {
                Write-M365DSCHost -Message "`r`n" -DeferWrite
                Write-M365DSCHost -Message "    |---[$index/$($excludes.Count)] $($exclude.Id)" -DeferWrite

                $results = @{
                    Id                                        = $exclude.Id
                    PermissionGrantPolicyId                   = $policy.Id
                    PermissionType                            = $exclude.PermissionType
                    ResourceApplication                       = $exclude.ResourceApplication
                    Permissions                               = $exclude.Permissions
                    PermissionClassification                  = $exclude.PermissionClassification
                    ClientApplicationIds                      = $exclude.ClientApplicationIds
                    ClientApplicationTenantIds                = $exclude.ClientApplicationTenantIds
                    ClientApplicationPublisherIds             = $exclude.ClientApplicationPublisherIds
                    ClientApplicationsFromVerifiedPublisherOnly = $exclude.ClientApplicationsFromVerifiedPublisherOnly
                    Ensure                                    = 'Present'
                    Credential                                = $Credential
                    ApplicationId                             = $ApplicationId
                    TenantId                                  = $TenantId
