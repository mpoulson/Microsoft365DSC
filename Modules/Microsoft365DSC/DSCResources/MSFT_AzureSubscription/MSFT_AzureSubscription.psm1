    ...
    try
    {
        if ($null -eq $Script:exportedInstances -or -not $Script:ExportMode)
        {
            ...
            if (-not [System.String]::IsNullOrEmpty($Id))
            {
                ...
                $uri = (Get-M365DSCAzureManagementUri) + "$($InvoiceSectionId)/billingSubscriptions/$($Id)?api-version=2024-04-01"
            ...
            {
                $uri = (Get-M365DSCAzureManagementUri) + "$($InvoiceSectionId)/billingSubscriptions?api-version=2024-04-01"
                $response = Invoke-AzRest -Uri $uri -Method Get
...
function Set-TargetResource
{
    ...
    {
        $uri = (Get-M365DSCAzureManagementUri) + "/providers/Microsoft.Subscription/aliases/$((New-Guid).ToString())?api-version=2021-10-01"
        $params = @{
            properties = @{
    ...
    elseif ($Ensure -eq 'Absent' -and $currentInstance.Ensure -eq 'Present')
    {
        ...
        $uri = (Get-M365DSCAzureManagementUri) + "/subscriptions/$($currentInstance.Id)/providers/Microsoft.Subscription/cancel?api-version=2019-03-01-preview&ImmediateDelete=true"
...
function Export-TargetResource
{
    ...
    try
    {
        ...
        $uri = (Get-M365DSCAzureManagementUri) + "/providers/Microsoft.Billing/billingaccounts/?api-version=2020-05-01"
        $response = Invoke-AzRest -Uri $uri -Method Get
        ...
        {
            $uri = (Get-M365DSCAzureManagementUri) + "/providers/Microsoft.Billing/billingaccounts/$($billingAccount.Name)/billingprofiles/?api-version=2020-05-01"
            $response = Invoke-AzRest -Uri $uri -Method Get
            $billingProfiles = (ConvertFrom-Json $response.Content).value

            foreach ($billingProfile in $billingProfiles)
            {
                $uri = (Get-M365DSCAzureManagementUri) + "/providers/Microsoft.Billing/billingAccounts/$($billingAccount.name)/billingProfiles/$($billingProfile.name)/billingSubscriptions?api-version=2024-04-01"
                $response = Invoke-AzRest -Uri $uri -Method Get
