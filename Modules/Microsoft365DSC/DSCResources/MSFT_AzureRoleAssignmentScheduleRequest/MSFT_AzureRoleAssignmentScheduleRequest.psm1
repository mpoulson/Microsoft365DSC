...
function Get-TargetResource
{
    ...
    try
    {
        if (-not $Script:exportedInstance)
        {
            ...
            {
                Write-Verbose -Message 'Retrieving all Azure role assignment schedules for scope'
        ...
        $results = @{
            ...
            Justification         = "Assignment of Azure role '$RoleDefinitionName' to principal '$Principal'"
...
function Set-TargetResource
{
    ...
    {
        Write-Verbose -Message "Creating new Azure PIM role assignment schedule"
        $requestBody.properties.requestType = 'AdminAssign'

        $requestId = (New-Guid).ToString()
        $uri = "$azureManagementEndpoint$Scope/providers/Microsoft.Authorization/roleAssignmentScheduleRequests/$($requestId)?api-version=2020-10-01"
    ...
    {
        Write-Verbose -Message "Updating Azure PIM role assignment schedule"
    ...
    {
        Write-Verbose -Message "Removing Azure PIM role assignment schedule"
        ...
        foreach ($config in $Script:exportedInstances)
        {
            ...
            if ($Results.ScheduleInfo)
            {
                $complexMapping = @(
                    @{
                        Name            = 'expiration'
                        CimInstanceName = 'AzureRoleAssignmentScheduleRequestScheduleExpiration'
                        IsRequired      = $False
                    }
                )
                $complexTypeStringResult = Get-M365DSCDRGComplexTypeToString -ComplexObject $Results.ScheduleInfo `
                        -CIMInstanceName 'AzureRoleAssignmentScheduleRequestSchedule' `
